...
By default (`--snapshot-bootstrap-mode solanacdn-snapshots`), the validator bootstraps snapshots via
the Pipe snapshot manifest service using:

- `--snapshot-manifest-url` (default: `https://data.pipedev.network/snapshot-manifest.json`)

To use the legacy gossip-based snapshot discovery + RPC snapshot download flow, either:

- pass `--snapshot-bootstrap-mode gossip-snapshots`, or
- shortcut: pass `--snapshot-manifest-url false` (forces gossip snapshots)
