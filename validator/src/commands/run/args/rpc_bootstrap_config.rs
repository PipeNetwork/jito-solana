use {
    crate::{
        bootstrap::{RpcBootstrapConfig, SnapshotBootstrapMode},
        commands::{FromClapArgMatches, Result},
    },
    clap::{value_t, values_t, Arg, ArgMatches},
...
impl Default for RpcBootstrapConfig {
    fn default() -> Self {
        Self {
            ...
            snapshot_bootstrap_mode: SnapshotBootstrapMode::SolanaCdnSnapshots,
...
impl FromClapArgMatches for RpcBootstrapConfig {
    fn from_clap_arg_match(matches: &ArgMatches) -> Result<Self> {
        ...
        let snapshot_manifest_url = matches
            .value_of("snapshot_manifest_url")
            .unwrap_or(DEFAULT_SNAPSHOT_MANIFEST_URL)
            .trim()
            .to_string();

        let snapshot_bootstrap_mode = if snapshot_manifest_url.eq_ignore_ascii_case("false") {
            SnapshotBootstrapMode::GossipSnapshots
        } else {
            match matches
                .value_of("snapshot_bootstrap_mode")
                .unwrap_or("solanacdn-snapshots")
            {
                "gossip-snapshots" => SnapshotBootstrapMode::GossipSnapshots,
                _ => SnapshotBootstrapMode::SolanaCdnSnapshots,
            }
        };
...
pub(crate) fn args<'a, 'b>() -> Vec<Arg<'a, 'b>> {
    vec![
        ...
        Arg::with_name("snapshot_bootstrap_mode")
            .long("snapshot-bootstrap-mode")
            .takes_value(true)
            .value_name("MODE")
            .possible_values(&["solanacdn-snapshots", "gossip-snapshots"])
            .default_value("solanacdn-snapshots")
            .help("Snapshot bootstrap mode: solanacdn-snapshots (default) or gossip-snapshots (legacy)"),
        ...
        Arg::with_name("snapshot_manifest_url")
            ...
            .help("Snapshot manifest URL (Pipe snapshot service). Set to 'false' to use legacy gossip snapshots"),
