...
use solanacdn_protocol::messages::{
    ...
    AuthWithSessionToken, ControlRequest, ControlResponse, Heartbeat, HeartbeatStats, PopToAgent,
...
pub struct SolanaCdnStatus {
    ...
    pub rx_tx_packets_total: u64,
    pub tx_injected_packets_total: u64,
    pub tx_inject_failed_total: u64,
...
impl SolanaCdnHandle {
    ...
    pub fn status_snapshot(&self) -> SolanaCdnStatus {
        ...
        let rx_tx_packets_total = self.rx_tx_packets.load(Ordering::Relaxed);
        let tx_injected_packets_total = self.tx_injected_packets.load(Ordering::Relaxed);
        let tx_inject_failed_total = self.tx_inject_failed.load(Ordering::Relaxed);
        ...
        SolanaCdnStatus {
            ...
            rx_tx_packets_total,
            tx_injected_packets_total,
            tx_inject_failed_total,
...
fn format_prometheus_metrics(handle: &SolanaCdnHandle) -> String {
    ...
    out.push_str(
        "# HELP solanacdn_rx_tx_packets_total Total transaction packets received from POPs
",
    );
    out.push_str(
        "# TYPE solanacdn_rx_tx_packets_total counter
",
    );
    out.push_str(&format!(
        "solanacdn_rx_tx_packets_total {}
",
        status.rx_tx_packets_total
    ));

    out.push_str("# HELP solanacdn_tx_injected_packets_total Total transaction packets injected into the validator
");
    out.push_str(
        "# TYPE solanacdn_tx_injected_packets_total counter
",
    );
    out.push_str(&format!(
        "solanacdn_tx_injected_packets_total {}
",
        status.tx_injected_packets_total
    ));

    out.push_str(
        "# HELP solanacdn_tx_inject_failed_total Total transaction packets that failed injection
",
    );
    out.push_str(
        "# TYPE solanacdn_tx_inject_failed_total counter
",
    );
    out.push_str(&format!(
        "solanacdn_tx_inject_failed_total {}
",
        status.tx_inject_failed_total
    ));

    out.push_str("# HELP solanacdn_tx_deduped_packets_total Total SolanaCDN transaction packets dropped due to deduplication
");
    out.push_str(
        "# TYPE solanacdn_tx_deduped_packets_total counter
",
    );
    out.push_str(&format!(
