...
impl Consumer {
    ...
    fn execute_and_commit_transactions_locked(
        ...
        let mut processed_transaction_bytes = 0u64;
            ...
            processing_results
                ...
                .filter_map(|(processing_result, tx)| {
                    if processing_result.was_processed() {
                        processed_transaction_bytes += tx.serialized_size() as u64;
        ...
        let entry_bytes = if batches.is_empty() {
            0
        } else {
            SERIALIZED_ENTRIES_OVERHEAD
                + ((batches.len() - 1) as u64 * 48)
                + processed_transaction_bytes
        };
        let reserved_bytes =
        ...
        let (recording_result, starting_transaction_index) = match record_transactions_summary {
            Ok(summary) => {
                ...
                (record_transactions_result, starting_transaction_index)
            }
